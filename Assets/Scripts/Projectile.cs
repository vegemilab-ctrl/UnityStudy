...
public class Projectile : MonoBehaviour
{
    ...
    [Space(10f), Header("공격 관련")]
    public Rigidbody _rigidbody; //물리 충돌 및 이동하기 위한 리지드바디
    [Range(1, 20)]
    ...
    public void Launch()
    {
        //절대적인 방향으로 힘을 가함.(예 : 세상읭 정면 방향으로 힘을 가해라)
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        else
        {
            ...
            //검출된 적에게 데미지를 주고 파괴된 적에게 물리적인 폭발력을 주어 날아가게 만듦.
            for (int i = 0; i < colliders.Length; i++)
            {
                ...
                if (rb != null && rb.TryGetComponent<Enemy>(out var e))
                {
                    if (e.Hit(damage))
                    {
                        //날아가려면 부모와 떨어져야하기 때문에 부모와 독립시킴
                        rb.transform.SetParent(null);
                        //폭발범위에 따른 힘을 가해 날아가게 만듦
